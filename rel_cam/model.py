...
from rel_cam.modules.resnet import resnet18
...
class PatchClassifier(nn.Module):
    def __init__(self, out_classes=config.NUM_CLASSES):
        super().__init__() 
        # maybe first load pretrained model, then append fc layer
        self.model = resnet18(pretrained=False, num_classes=out_classes)
...
class ModelHandler():
    ...
    def predict(self, pil_img, show_results=False, label_dict=None, fname=None):
        ...
        with torch.no_grad():            
            ...
            return prediction, prob, label_dict[prediction] if label_dict is not None else None


if __name__ == '__main__':
    model = PatchClassifier(
        out_classes=5
    ).eval().to(config.ACCELERATOR)
